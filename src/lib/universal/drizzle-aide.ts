...
export const snakeToCamelCase = (text: string) => text.toLowerCase().replace(
...
export function tableBuilderAide<
	...
	const TBA = {
		...
		primaryKeyFK,
		atTimestamp,
		indexName: (...columns: AnySQLiteColumn[]) => `${tableName}_${columns.map(c => c.name).join('_')}_idx`,
		uniqueIndexName: (...columns: AnySQLiteColumn[]) => `${tableName}_${columns.map(c => c.name).join('_')}_unq_idx`,
		lintTable: (table: AnySQLiteTable, helpers: {
			...
			for(const colEntry of Object.entries(columns)) {
				...
				if(column.primary && (column.name != TBA.primaryKeyColName && column.name != "id") && !(ignore?.primaryKeyNameConsistency?.(column))) {
					encounterIssue({ issue: `${tableName}.${column.name} is a primary key, it should be named ${tableName}.${TBA.primaryKeyColName} NOT ${column.name}` });
				}
				
				if(!column.primary && !(ignore?.propNameConsistency?.(column))) {
					const shouldBe = snakeToCamelCase(column.name);
					if(propName != shouldBe) {
						encounterIssue({ issue: `${table}.${propName} property should be named ${table}.${shouldBe} NOT ${propName}` });
					}
				}				
			}
		},		
