...
export async function namespaceContent(conn: SqlConnState, name: string): Promise<NamespaceContent | undefined> {
...
export const gitLabNamespaceQR = (conn: SqlConnState, name: string) => conn.memoizableSqlResults(async () => {
...
export interface ContentCollections {
  ...
  readonly pathTree: t.PathTree<{cc: ContentCollection; content: Content}, Pick<Namespace, "name">>;
}
...
export class TypicalContentCollections implements ContentCollections {
  static readonly singleton = (conn: SqlConnState, ns: Namespace) => m.singleton(async () => {
    const assignments = new TypicalContentCollections(ns);
    await assignments.populate(conn);
    return assignments;
  });
  readonly collections: ContentCollection[] = [];
  readonly pathTree: t.PathTree<{cc: ContentCollection; content: Content}, Pick<Namespace, "name">> = { children: [] };
  ...
  async populate(conn: SqlConnState): Promise<ContentCollections> {
    ...
    for (const row of assignments) {
      ...
      ac.register(row);
      t.populatePathTree({ terminal: {cc: ac, content: row} }, this.pathTree, 
...
export const gitLabIssuesQR = (conn: SqlConnState, namespace: Namespace) => conn.memoizableSqlResults(async () => {
...
export class TypicalUsers implements Users {
  ...
  async populate(conn: SqlConnState): Promise<Users> {
    ...
    for (
      const user of analytics.sort((a, b) => {
...
export const gitLabUsersAnalyticsQR = (conn: SqlConnState, namespace: Namespace) => conn.memoizableSqlResults(async () => {
  return await conn.SQL<{
    id: number;
    email: string;
    name: string;
