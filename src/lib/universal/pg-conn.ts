...
export const dbConfigureFromText = (configSupplier: () => string | undefined): DatabasesConfiguration => {
	...
	const simpleTextConn = (text: string): DatabasesConfiguration => {
		return { isConfigured: true, configTextSupplied, connURLs: { defaultConnID: PostgreSqlDefaultConnID, URLs: { [PostgreSqlDefaultConnID]: text } }, defaultConnURL: text };
	}
			...
			try {
				...
				if (typeof parsed === "string" && parsed.match(pgConnUrlPattern)) {
					...
					if (typeof parsed == "object") {
						...
						return { isConfigured: true, configTextSupplied, connURLs: conns, defaultConnURL: parsed.conns[parsed.defaultConnID] };
					} else {
						return { isConfigured: false, configTextSupplied, configTextParseError: new Error("Expected single JSON object, got " + (typeof parsed)) };
					}
				}
			} catch (err) {
...
export const mssFactory = ({ isDbAvailable, mssStatsInstances = new Map() }: {
	...
	const mssFileNameSupplier = (key: string) => <V>(_value?: V) =>
		path.join(process.cwd(), 'src', 'cache', 'memoized-sql-results', key.replace(invalidChars, '-')) + ".memoized.json";
...
export const memoizableSqlResults = (options: { readonly isDbAvailable: () => boolean; }) => {
	...
	return <A extends unknown[], V>(queryFn: (...args: A) => Promise<V>, key: string): (...args: A) => Promise<V> => {
		let instance = instances.get(key);
		if (!instance) {
	...
	return () => {
		...
		factory = {
			...
			conn: async (connID: string = PostgreSqlDefaultConnID): Promise<SqlConnState> => {
				...
				if (!connURL) {
					const SQL = postgres();
					return { connID, state: "invalid connection ID", SQL, memoizableSqlResults: memoizableSqlResults({ isDbAvailable: () => false }) }
				}
				let connState = connSqlInstances.get(connURL);
				if (!connState) {
					try {
						const SQL = postgres(connURL ? connURL : "postgres://localhost/unknown_db", { connection: { application_name: `postgres.js::astro` } })
						...
						if (testResult && testResult.length == 1) {
							...
							const SQL = postgres();
							connState = { connID, state: "unknown", SQL, memoizableSqlResults: memoizableSqlResults({ isDbAvailable: () => false }) };
						}
					} catch (err) {
						...
						const SQL = postgres();
						connState = { connID, state: "invalid connection", SQL, memoizableSqlResults: memoizableSqlResults({ isDbAvailable: () => false }) };
					}
...
export const dbConnsFactory = (() => {
	const cf = prepareConnsFactory(() => {
		const result = dbConfigureFromText(() => process.env.SERVICE_DB_CONN_URLS ?? import.meta.env.SERVICE_DB_CONN_URLS);
