...
export const foreignContentMssFactory = <V>({ fsssFactory, isForeignContentAvailable, expireDurationMS, fileSuffix, mssStatsInstances = new Map() }: {
	...
	const mssFileNameSupplier = (key: string) => <V>(_value?: V) =>
		path.join(process.cwd(), 'src', 'cache', 'memoized-foreign-content', key.replace(invalidChars, '-').replaceAll(/\-+/g, "-")) + fileSuffix;
...
export const foreignQueryableHtmlMemoizer = (fqhmOptions: {
	...
	return (url: string, args?: {
		...
		if (!instance) {
			instance = m.memoize(
				async () => {
					const dom = await (args?.sanitized ? fc.queryableSanitizedContent(url) : fc.queryableContent(url));
...
export const foreignReadableHtmlMemoizer = (options: {
	...
	const instances = new Map<string, (...args: any) => Promise<any>>();
	// the Promise type of the return should match Mozilla Readability result
	return (url: string, key = url): () => Promise<{
		/** article title */
		title: string;
		/** author metadata */
		byline: string;
		...
		if (!instance) {
			instance = m.memoize(
				async () => await fc.readableContent(url), foreignContentMssFactory({
...
export const memoizableForeignHTML = foreignQueryableHtmlMemoizer({ isForeignContentAvailable: () => true });
...
export const memoizableForeignReadable = foreignReadableHtmlMemoizer({ isForeignContentAvailable: () => true });
...
export async function memoizableForeignContent(foreignContent: fc.ForeignContent) {
	if(!foreignContent.content || foreignContent.content.readable) {
		return (await memoizableForeignReadable(foreignContent.url)())?.content ?? `memoizableForeignContent:readable-failed`;
	}
	...
	if(fcc.selectFirst || fcc.selectAll) {
		return await memoizableForeignHTML(foreignContent.url, { 
			sanitized, extractHTML: (dom) => {
				if(fcc.selectFirst) {
					const first = dom.window.document.querySelector(fcc.selectFirst);
					return first ? first.outerHTML : (fcc.onSelectFail ?? 'memoizableForeignContent:selectFirst-failed');
				}
				if(fcc.selectAll) {
					let text = '';
					...
					return text.length > 0 ? text : (fcc.onSelectFail ?? 'memoizableForeignContent:selectAll-failed');
				}
