...
export async function walkFsEntries(startDir: string, options?: {
  ...
  for (const entry of entries) {
    ...
    if (stat.isDirectory()) {
      const instruction = onDirectory ? await (onDirectory?.({ startDir, parentRelPath, absPath, relPath, stat, level }) ?? "enter") : "enter";
      switch (instruction) {
        case "enter":
      ...
      const instruction = onFile ? await onFile({ startDir, parentRelPath, absPath, relPath, stat, level }) : "continue";
